...
memdat <- read.csv('data/exp2/preproc_mem_data.csv')
mathdat <- read.csv('data/exp2/preproc_math_data.csv')
testdat <- read.csv('data/exp2/preproc_test_data.csv')
...
ggsave('figures/exp2_recall_by_list_and_condition.png', width=6.5, height=3, units='in')
...
memdat %>% 
  ...
  theme_bw() +
  ggtitle('Experiment 2')
ggsave('figures/exp2_studyRTs_by_current_and_previous_item_instructions.png', width= 5, height=3, units='in')
...
ggsave('figures/exp2_acc_f(studyRTs).png', width=4.2, height=3, units='in')
...
ml31 <- brm(studyRT ~ 1 + (trial_condition|id), 
            data=mldat3, save_all_pars = TRUE, iter = 4000, cores = 3, chains = 3)
...
ml44_1a <- brm(acc1 ~ trial_condition + (trial_condition * rt||id), 
               data=filter(mldat4), family=bernoulli(), save_all_pars = TRUE, iter = 4000, cores = 3, chains = 3)

ml44_2a <- brm(acc1 ~ trial_condition + rt + (trial_condition + rt||id), 
               data=filter(mldat4), save_all_pars = TRUE, iter = 4000, cores = 3, chains = 3)

ml44_3a <- brm(acc1 ~ trial_condition * rt + (trial_condition + rt||id), 
               data=filter(mldat4), save_all_pars = TRUE, iter = 4000, cores = 3, chains = 3)

ml44_1r <- brm(acc1 ~ 1 + (rt|id), 
               data=filter(mldat4, trial_condition == "remember"), family=bernoulli(), save_all_pars = TRUE, iter = 4000, cores = 3, chains = 3)

ml44_1p <- brm(acc1 ~ 1 + (rt|id), 
               data=filter(mldat4, trial_condition == "process"), family=bernoulli(), save_all_pars = TRUE, iter = 4000, cores = 3, chains = 3)

ml44_2r <- brm(acc1 ~ rt + (rt|id), 
               data=filter(mldat4, trial_condition == "remember"), family=bernoulli(), save_all_pars = TRUE, iter = 4000, cores = 3, chains = 3, 
               prior  = set_prior('student_t(3, 0, 2.5)', class = 'b'))

ml44_2p <- brm(acc1 ~ rt + (rt|id), 
               data=filter(mldat4, trial_condition == "process"), family=bernoulli(), save_all_pars = TRUE, iter = 4000, cores = 3, chains = 3, 
               prior  = set_prior('student_t(3, 0, 2.5)', class = 'b'))
...
bf4_a32 <- bayes_factor(ml44_3a, ml44_2a)
bf4_r
bf4_p
bf4_a
bf4_a32