...
memdat <- read.csv('data/exp4/preproc_mem_data.csv')
mathdat <- read.csv('data/exp4/preproc_math_data.csv')
...
memdat %>% 
  filter(!exclude, overall_recog_acc < 1) %>% 
  group_by(id,listid, trial_condition) %>% 
  summarise(acc = mean(ifelse(listid == 3, recog_acc, recall_acc))) %>% 
  group_by(listid, trial_condition) %>% 
  summarise(acc_se = sd(acc)/sqrt(length(acc)),
...
memdat %>% 
  ...
  group_by(id,listid, trial_condition, test_type) %>% 
  summarise(acc = mean(acc)) %>% 
  ggplot(aes(listid, acc, color=trial_condition, fill=trial_condition)) +
  geom_point(position = position_jitter(width=0.2), alpha=0.1, size=2) +
  stat_summary(geom="line") +
  stat_summary(geom="errorbar", width=0.1, color='black') +
  stat_summary(geom="point", color='black', shape=21) +
...
ggsave('figures/exp8_recall_recog_by_list_and_condition.png', width=6.5, height=3, units='in')
