...
memdat <- read.csv('data/exp12/preproc_mem_data.csv')
mathdat <- read.csv('data/exp12/preproc_math_data.csv')
...
memdat %>% 
  ...
  ggplot(aes(listid, recall_acc, fill=btw_cond, color=btw_cond, label=recall_acc)) +
...
memdat %>% 
  ...
  ggplot(aes(listid, rt, color=btw_cond)) +
  stat_summary(geom="pointrange") +
  stat_summary(geom="line") +
  theme_bw() +
  theme(panel.grid.major.x = element_blank(),
...
memdat %>% 
  ...
  group_by(id, listid, btw_cond) %>% 
  summarise(loc_acc = mean(loc_acc)) %>% 
  ggplot(aes(listid, loc_acc, color=btw_cond, fill=btw_cond, group=btw_cond)) +
  ...
  ggtitle('Experiment 12') 
ggsave('figures/exp12_source_memory_acc.png', width=5, height=3.5)

glmer(loc_acc ~ btw_cond + (1|id), data=filter(memdat, loc_rt > 500, !exclude, listid==2), family="binomial") %>% summary()
...
memdat %>% 
  ...
  ggplot(aes(size_group, as.numeric(resp == "larger"), color=btw_cond)) +
  stat_summary(geom="pointrange") +
  stat_summary(geom="line") +
  coord_cartesian(ylim=c(0,1))
...
memdat %>% 
  # filter(stimulus_condition == "words") %>% 
  ggplot(aes(size_group, recall_acc, color=btw_cond)) +
  stat_summary(geom="pointrange") +
  stat_summary(geom="line") +
  coord_cartesian(ylim=c(0,1)) +
  facet_wrap(~resp)
